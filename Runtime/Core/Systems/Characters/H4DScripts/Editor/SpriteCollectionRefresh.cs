...
namespace HeroEditor4D.Common.Editor
{
    ...
    public class SpriteCollectionRefresh : AssetPostprocessor
    {
        ...
        public static void Refresh(SpriteCollection spriteCollection)
        {
            ...
            spriteCollection.MeleeWeapon1H = LoadSprites(
                spriteCollection,
                "/Equipment/MeleeWeapon1H",
                1
            );
            spriteCollection.MeleeWeapon2H = LoadSprites(
                spriteCollection,
                "/Equipment/MeleeWeapon2H",
                1
            );
            ...
            if (spriteCollection.DebugLogging)
                Debug.Log("<color=yellow>SpriteCollection: refreshed.</color>");
        }

        private static List<ItemSprite> LoadSprites(
            SpriteCollection sc,
            string subPath,
            int nesting = 0
        )
        {
            ...
            foreach (var asset in sc.SpriteFolders)
            {
                ...
                var exclude = sc
                    .CollectionFilter.Where(i => i.StartsWith("!"))
                    .Select(i => i.Replace("!", ""))
                    .ToList();
                ...
                foreach (var ext in extensions)
                {
                    var entries = Directory
                        ...
                        .Select(p => p.Replace("\\", "/"))
                        .Select(p => new ItemSprite(
                            GetEditionName(p),
                            GetCollectionName(p, nesting),
                            type,
                        ...
                        .Where(i =>
                            ...
                            || sc.CollectionFilterIgnore.Any(i.Path.Contains)
                                && !exclude.Contains(i.Collection)
                        )
                        .ToList();
