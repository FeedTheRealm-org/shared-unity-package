    ...
    public abstract class ItemWorkspace : MonoBehaviour
    {
        ...
        )
        {
            ...
            if (from.Stacked)
            {
                ...
                {
                    SelectedItem =
                        currencyId == null ? moved : from.Items.Single(i => i.Id == currencyId);
                }
            }
            else
            {
        ...
        {
            if (item.Count <= 0)
                throw new ArgumentException("item.Count <= 0");
            if (amount <= 0)
                throw new ArgumentException("amount <= 0");
            if (item.Count < amount)
                throw new ArgumentException("item.Count < amount");
            ...
            if (to.Stacked)
            {
                ...
                switch (targets.Count)
                {
                    ...
                    default:
                        throw new Exception(
                            $"Unable to move item silently: {item.Id} ({item.Hash})."
                        );
                }
            }
            else
