...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    public class ScrollInventory : ItemContainer
    {
        ...
        public Func<Item, int> SortingFunc = item =>
            TypePriority.Contains(item.Params.Type) ? TypePriority.IndexOf(item.Params.Type) : 999; // You can override this.
        ...
        public override void Refresh(Item selected)
        {
            ...
            for (var i = 0; i < items.Count; i++)
            {
                ...
                if (AutoSelect)
                    instance.Select(items[i] == selected);
            }
            ...
            switch (Grid.constraint)
            {
                case GridLayoutGroup.Constraint.FixedColumnCount:
                {
                    var height = Mathf.FloorToInt(
                        (ScrollRect.GetComponent<RectTransform>().rect.height + Grid.spacing.y)
        ...
        {
            return _itemInstances.SingleOrDefault(i =>
                i.gameObject.activeSelf && i.Item != null && i.Item.Hash == item.Hash
            );
        }
        ...
        {
            return _itemInstances.SingleOrDefault(i =>
                i.gameObject.activeSelf && i.Item != null && i.Item.Id == itemId
            );
        }
        ...
        )
        {
            ...
            if (!vertical)
                pos.y = ScrollRect.content.anchoredPosition.y;
