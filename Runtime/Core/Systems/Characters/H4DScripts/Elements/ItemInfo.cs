...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    public class ItemInfo : MonoBehaviour
    {
        ...
        public virtual void Initialize(Item item, int price, bool trader = false)
        {
            ...
            if (item.Params.Class != ItemClass.Undefined)
                main.Add(item.Params.Class);
            ...
            {
                { "ItemInfo.Type", string.Join(" / ", main) },
            };

            if (item.Params.Level >= 0)
                dict.Add("ItemInfo.Level", item.Params.Level);
