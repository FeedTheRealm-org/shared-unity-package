...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Elements
{
    ...
    public class Equipment : ItemContainer
    {
        ...
        public void SetBagSize(int size)
        {
            ...
            var supplies = GetComponentsInChildren<ItemSlot>(true)
                .Where(i => i.Types.Contains(ItemType.Supply))
                .ToList();
        ...
        private Item FindItem(ItemSlot slot)
        {
            ...
            {
                var copy = Items.SingleOrDefault(i =>
                    i.Params.Type == ItemType.Weapon && (i.IsTwoHanded || i.IsFirearm)
                );
