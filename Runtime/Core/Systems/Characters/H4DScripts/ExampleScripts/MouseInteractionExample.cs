    ...
    public class MouseInteractionExample : MonoBehaviour
    {
        ...
        private void SetDirection()
        {
            var position = (Vector2)
                Camera.main.WorldToScreenPoint(Character.Active.AnchorBody.position);
        ...
        private float GetArmRotation(Transform armR) // You should be a geometry ninja to understand this.
        {
            ...
            var elbow = (Vector2)
                muzzle.TransformPoint(0, magnitude * Mathf.Cos(alpha * Mathf.Deg2Rad), 0);
            var angle = Vector2.SignedAngle(
                (Vector2)muzzle.position - elbow,
