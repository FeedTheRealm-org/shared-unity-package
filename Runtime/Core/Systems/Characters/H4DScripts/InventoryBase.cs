...
namespace Assets.HeroEditor4D.InventorySystem.Scripts
{
    ...
    public class InventoryBase : ItemWorkspace
    {
        ...
        public void Initialize(
            ref List<Item> inventory,
            ref List<Item> equipped,
            int bagSize,
            Action onRefresh
        )
        {
        ...
        public void Equip()
        {
            ...
            var equipped = SelectedItem.IsFirearm
                ...
                : Equipment
                    .Items.Where(i => i.Params.Type == SelectedItem.Params.Type && !i.IsFirearm)
                    .ToList();
        ...
        public void Craft()
        {
            ...
            {
                materials.ForEach(i =>
                    PlayerInventory.Items.Single(j => j.Hash == i.Hash).Count -= i.Count
                );
                PlayerInventory.Items.RemoveAll(i => i.Count == 0);

                var itemId = SelectedItem.Params.FindProperty(PropertyId.Craft).Value;
                var existed = PlayerInventory.Items.SingleOrDefault(i =>
                    i.Id == itemId && i.Modifier == null
                );
            ...
            if (receipt)
            {
                if (LearnButton == null)
                {
                    var materialSelected =
                        !PlayerInventory.Items.Contains(SelectedItem)
        ...
        private List<Item> MaterialList =>
            SelectedItem
                .Params.FindProperty(PropertyId.Materials)
                .Value.Split(',')
                .Select(i => i.Split(':'))
        ...
        {
            return PlayerInventory.Items.Contains(SelectedItem)
                && Equipment.Slots.Any(i => i.Supports(SelectedItem));
        }
        ...
        {
            return materials.All(i =>
                PlayerInventory.Items.Any(j => j.Hash == i.Hash && j.Count >= i.Count)
            );
        }
        ...
        private void AutoRemove(List<Item> items, int max = 1)
        {
            ...
            {
                MoveItemSilent(
                    items.LastOrDefault(i => i.Id != SelectedItem.Id) ?? items.Last(),
                    Equipment,
                    PlayerInventory
                );
            }
