...
namespace Assets.HeroEditor4D.InventorySystem.Scripts
{
    ...
    public class ShopBase : ItemWorkspace
    {
        ...
        {
            if (item.Params.Rarity > ItemRarity.Common)
                return 2;
        ...
        public void SelectItem(Item item)
        {
            ...
            ItemInfo.Initialize(
                ...
                TraderInventory.Items.Contains(item)
            );
            Refresh();
        }
        ...
        {
            if (
                !BuyButton.gameObject.activeSelf
            ...
            if (GetCurrency(PlayerInventory, CurrencyId) < price)
            {
                ...
                TapHeroes.Scripts.Interface.Popup.Instance.ShowMessage(
                    SimpleLocalization.LocalizationManager.Localize(
                        "Common.NoFunds",
                        "["
                            + TapHeroes.Scripts.Extensions.GetLocalizedName(
                                new Item(CurrencyId).Params
                            )
                            + "]"
                    ),
                    CurrencyId,
        ...
        {
            if (
                !SellButton.gameObject.activeSelf
        ...
        private void SetAmount(int amount)
        {
            ...
            ItemInfo.UpdatePrice(
                SelectedItem,
                GetPrice(SelectedItem),
                TraderInventory.Items.Contains(SelectedItem)
            );
        }
        ...
        {
            BuyButton.SetActive(
                SelectedItem.Params.Type != ItemType.Currency
                    && SelectedItem.Count > 0
                    && !SelectedItem.Params.Tags.Contains(ItemTag.NotForSale)
                    && !SelectedItem.Params.Tags.Contains(ItemTag.Quest)
                    && CanBuy(SelectedItem)
            );
            SellButton.SetActive(false);
        ...
        {
            BuyButton.SetActive(false);
            SellButton.SetActive(
                SelectedItem.Count > 0
                    && !SelectedItem.Params.Tags.Contains(ItemTag.NotForSale)
                    && !SelectedItem.Params.Tags.Contains(ItemTag.Quest)
                    && SelectedItem.Id != CurrencyId
                    && CanSell(SelectedItem)
            );
