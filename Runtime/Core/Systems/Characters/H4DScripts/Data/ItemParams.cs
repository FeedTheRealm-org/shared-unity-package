...
namespace Assets.HeroEditor4D.InventorySystem.Scripts.Data
{
    ...
    public class ItemParams
    {
        ...
        {
            var target = Properties.SingleOrDefault(i =>
                i.Id == id && i.Element == ElementId.Physic
            );
        ...
        {
            var localized =
                Localization.SingleOrDefault(i => i.Language == language)
        ...
        public List<string> MetaToList()
        {
            return Meta.IsEmpty()
                ? new List<string>()
                : JsonConvert.DeserializeObject<List<string>>(Meta);
        }
