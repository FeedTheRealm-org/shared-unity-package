...
namespace Assets.HeroEditor4D.Common.Scripts.CharacterScripts
{
    ...
    public partial class Character : MonoBehaviour
    {
        ...
        private void TryInitialize()
        {
            ...
            if (
                !expressionNames.Contains("Default")
                || !expressionNames.Contains("Angry")
            ...
            if (ShowHelmet)
            {
                ...
                if (Helmet == null || EquipmentTags.Contains(EquipmentTag.FullHair))
                    ears = !EquipmentTags.Contains(EquipmentTag.HideEars);
                else
                    ears = EquipmentTags.Contains(EquipmentTag.ShowEars);

                MapSprites(EarsRenderers, ears ? Ears : null);
                SetSprite(
                    HairRenderer,
                    Hair == null ? null
                        : Helmet != null && !EquipmentTags.Contains(EquipmentTag.FullHair)
                            ? HairCropped
                        : Hair
                );
        ...
        private void MapSprite(SpriteRenderer spriteRenderer, List<Sprite> sprites)
        {
            SetSprite(
                ...
                sprites == null
                    ? null
                    : spriteRenderer.GetComponent<SpriteMapping>().FindSprite(sprites)
            );
        }
        ...
        public void CopyFrom(Character character)
        {
            if (character == null)
                throw new ArgumentNullException(
                    nameof(character),
                    "Can't copy from empty character!"
                );
            ...
            foreach (
                var target in GetComponentsInChildren<SpriteRenderer>(true)
            ...
            )
            {
                foreach (var source in character.GetComponentsInChildren<SpriteRenderer>(true))
                {
                    if (
                        ...
                        && target.transform.parent.name == source.transform.parent.name
                    )
                    {
