    ...
    public partial class Character
    {
        ...
        public void SetBody(ItemSprite item, BodyPart part, Color? color)
        {
            switch (part)
            {
                ...
                case BodyPart.Hair:
                    ...
                    if (EquipmentTags.Contains(EquipmentTag.HideEars))
                        EquipmentTags.Remove(EquipmentTag.HideEars);
                    if (item != null && item.Tags.Contains("HideEars"))
                        EquipmentTags.Add(EquipmentTag.HideEars);
                ...
                case BodyPart.Eyebrows:
                    if (EyebrowsRenderer)
                        Expressions[0].Eyebrows = EyebrowsRenderer
                            .GetComponent<SpriteMapping>()
                            .FindSprite(item?.Sprites);
                    break;
                case BodyPart.Eyes:
                    if (EyesRenderer)
                    {
                        Expressions[0].Eyes = EyesRenderer
                            .GetComponent<SpriteMapping>()
                            .FindSprite(item?.Sprites);
                        Expressions
                            .Where(i => i.Name != "Dead")
                ...
                case BodyPart.Mouth:
                    if (MouthRenderer)
                        Expressions[0].Mouth = MouthRenderer
                            .GetComponent<SpriteMapping>()
                            .FindSprite(item?.Sprites);
                    break;
                case BodyPart.Beard:
                ...
                case BodyPart.Makeup:
                    ...
                    break;
                default:
                    throw new NotImplementedException($"Unsupported part: {part}.");
            }
        ...
        public void SetExpression(string expression)
        {
            if (Expressions.Count < 3)
                throw new Exception(
                    "Character must have at least 3 basic expressions: Default, Angry and Dead."
                );
        ...
        public void Equip(ItemSprite item, EquipmentPart part, Color? color)
        {
            ...
            switch (part)
            {
                case EquipmentPart.Helmet:
                    if (EquipmentTags.Contains(EquipmentTag.ShowEars))
                        EquipmentTags.Remove(EquipmentTag.ShowEars);
                    if (EquipmentTags.Contains(EquipmentTag.FullHair))
                        EquipmentTags.Remove(EquipmentTag.FullHair);
                    if (item != null && item.Tags.Contains("ShowEars"))
                        EquipmentTags.Add(EquipmentTag.ShowEars);
                    if (item != null && item.Tags.Contains("FullHair"))
                        EquipmentTags.Add(EquipmentTag.FullHair);
                ...
                case EquipmentPart.MeleeWeapon1H:
                    ...
                    PrimaryWeaponRenderer.color =
                        color
                        ?? (
                            item != null && item.Tags.Contains("Paint")
                                ? PrimaryWeaponRenderer.color
                                : Color.white
                        );
                    if (WeaponType != WeaponType.Paired)
                ...
                case EquipmentPart.Earrings:
                    ...
                    EarringsRenderers[0].color = EarringsRenderers[1].color =
                        color ?? EarringsRenderers[0].color;
                    break;
        ...
        private void SetArmorParts(List<SpriteRenderer> renderers, List<Sprite> armor, Color? color)
        {
            ...
            foreach (var r in renderers)
            {
                ...
                Armor.RemoveAll(i =>
                    i.name == mapping.SpriteName || mapping.SpriteNameFallback.Contains(i.name)
                );
        ...
        private void SetFirearmMuzzle(ItemSprite item)
        {
            ...
            var p = new Vector2(
                bounds.min.x + mx * (bounds.max.x - bounds.min.x),
                bounds.min.y + my * (bounds.max.y - bounds.min.y)
            );

            if (PrimaryWeaponRenderer.flipX)
                p.x *= -1;
