...
namespace Assets.HeroEditor4D.Common.Scripts.CharacterScripts
{
    ...
    public class CharacterAppearance
    {
        ...
        {
            var hair = Hair.IsEmpty()
                ? null
                : character.SpriteCollection.Hair.Single(i => i.Id == Hair);
            ...
            {
                var beard = Beard.IsEmpty()
                    ? null
                    : character.SpriteCollection.Beard.Single(i => i.Id == Beard);

                character.Beard =
                    beard == null
                        ? null
                        : character
                            .BeardRenderer.GetComponent<SpriteMapping>()
                            .FindSprite(beard.Sprites);
                character.BeardRenderer.color = BeardColor;
            }

            character.Ears = Ears.IsEmpty()
                ? null
                : character.SpriteCollection.Ears.FindSprites(Ears);
            ...
            character.Head = character
                .HeadRenderer.GetComponent<SpriteMapping>()
                .FindSprite(body.Sprites);
