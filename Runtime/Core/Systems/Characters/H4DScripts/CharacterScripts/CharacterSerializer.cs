    ...
    public partial class Character
    {
        public string ToJson()
        {
            if (SpriteCollection == null)
                throw new Exception("SpriteCollection is null!");
            ...
            switch (WeaponType)
            {
                ...
                case WeaponType.Paired:
                    description.Add(
                        "SecondaryWeapon",
                        SpriteToString(SpriteCollection.Firearm1H, SecondaryWeaponRenderer)
                    ); // TODO:
                    break;
                case WeaponType.Bow:
                ...
                case WeaponType.Crossbow:
                    description.Add(
                        "Crossbow",
                        SpriteToString(SpriteCollection.Crossbow, CrossbowRenderers[0])
                    );
                    break;
                default:
            ...
            {
                description.Add(
                    $"Expression.{expression.Name}.Eyebrows",
                    SpriteToString(
                        SpriteCollection.Eyebrows,
                        expression.Eyebrows,
                        EyebrowsRenderer.color
                    )
                );
                description.Add(
                    $"Expression.{expression.Name}.Eyes",
                    SpriteToString(SpriteCollection.Eyes, expression.Eyes, EyesRenderer.color)
                );
                description.Add(
                    $"Expression.{expression.Name}.EyesColor",
                    "#" + ColorUtility.ToHtmlStringRGBA(expression.EyesColor)
                );
                description.Add(
                    $"Expression.{expression.Name}.Mouth",
        ...
        public void LoadFromJson(string json, bool silent)
        {
            ...
            EquipmentTags =
                description.ContainsKey("EquipmentTags") && description["EquipmentTags"] != ""
                    ? description["EquipmentTags"]
                        .Split(',')
                        .Select(Enum.Parse<EquipmentTag>)
                        .ToList()
                    : new List<EquipmentTag>();

            RestoreFromString(
                ref Makeup,
                MakeupRenderer,
                SpriteCollection.Makeup,
                description["Makeup"],
                silent
            );
            RestoreFromString(
            ...
            foreach (var key in description.Keys)
            {
                if (key.Contains("Expression."))
                {
                    ...
                    switch (expressionPart)
                    {
                        ...
                        case "Mouth":
                            RestoreFromString(
                                ref expression.Mouth,
                                MouthRenderer,
                                SpriteCollection.Mouth,
                                description[key]
                            );
                            break;
                        default:
            ...
            WeaponType = (WeaponType)
                Enum.Parse(typeof(WeaponType), description["WeaponType"], silent);

            if (description.ContainsKey("PrimaryWeapon"))
            {
                var weapon = RestoreFromString(
                    ref PrimaryWeapon,
                    PrimaryWeaponRenderer,
                    GetWeaponCollection(WeaponType),
                    description["PrimaryWeapon"],
                    silent
                );

                if (
                    weapon != null
                    && (WeaponType == WeaponType.Firearm1H || WeaponType == WeaponType.Firearm2H)
                )
                {
        ...
        private IEnumerable<ItemSprite> GetWeaponCollection(WeaponType weaponType)
        {
            ...
            {
                case WeaponType.Melee1H:
                    return SpriteCollection.MeleeWeapon1H;
                case WeaponType.Paired:
                    return SpriteCollection.MeleeWeapon1H;
                case WeaponType.Melee2H:
                    return SpriteCollection.MeleeWeapon2H;
                case WeaponType.Bow:
                    return SpriteCollection.Bow;
                case WeaponType.Crossbow:
                    return SpriteCollection.Crossbow;
                case WeaponType.Firearm1H:
                    return SpriteCollection.Firearm1H;
                case WeaponType.Firearm2H:
                    return SpriteCollection.Firearm2H;
                case WeaponType.Throwable:
                    return SpriteCollection.Throwable;
                default:
                    throw new NotSupportedException(weaponType.ToString());
            }
        }

        private static string SpriteToString(
            IEnumerable<ItemSprite> collection,
        ...
        private static string SpritesToString(
            IEnumerable<ItemSprite> collection,
        ...
        )
        {
            ...
            var entry = collection.SingleOrDefault(i =>
                i.Sprite == sprite || i.Sprites.Any(j => j == sprite)
            );
            ...
            var result =
                color == Color.white
                    ? entry.Id
                    : entry.Id + "#" + ColorUtility.ToHtmlStringRGBA(color);

            return result;
        }

        private ItemSprite RestoreFromString(
            ref Sprite sprite,
            SpriteRenderer renderer,
            IEnumerable<ItemSprite> collection,
            string serialized,
        ...
        )
        {
            ...
            switch (entries.Count)
            {
                ...
                case 0:
                    if (silent)
                        Debug.LogWarning("entries.Count = " + entries.Count);
                    else
                ...
                default:
                    if (silent)
                        ...
                        throw new Exception(
                            $"Multiple entries with id {id} found in SpriteCollection."
                        );
                    return null;
            }
        }

        private static void RestoreFromString(
            ref List<Sprite> sprites,
            List<SpriteRenderer> renderers,
            List<ItemSprite> collection,
            string serialized,
            bool silent = false
        )
        {
                ...
                switch (entries.Count)
                {
                    case 1:
                        ...
                        else
                        {
                            var sprite = renderers[i]
                                .GetComponent<SpriteMapping>()
                                .FindSprite(entries[0].Sprites);
                    ...
                    case 0:
                        if (silent)
                            ...
                            throw new Exception(
                                $"Entry with id {id} not found in SpriteCollection."
                            );
                        break;
                    default:
                        if (silent)
