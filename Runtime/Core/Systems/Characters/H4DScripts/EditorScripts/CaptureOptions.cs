...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    public class CaptureOptions : MonoBehaviour
    {
        ...
        public void Capture()
        {
            ...
            if (Ready.isOn)
                options.Add(new CaptureOption("Ready"));
            if (Walk.isOn)
                options.Add(new CaptureOption("Walk"));
            if (Run.isOn)
                options.Add(new CaptureOption("Run"));
            if (Slash.isOn)
                options.Add(
                    new CaptureOption(
                        "Idle",
                        FindFirstObjectByType<Character4D>().WeaponType == WeaponType.Melee2H
                            ? "Slash2H"
                            : "Slash1H"
                    )
                );
            if (Jab.isOn)
                options.Add(new CaptureOption("Idle", "Jab"));
            ...
            if (Cast.isOn)
                options.Add(new CaptureOption("Idle", "Cast"));
            if (Block.isOn)
                options.Add(new CaptureOption("ShieldBlock"));
            if (Jump.isOn)
                options.Add(new CaptureOption("Jump", "Idle"));
            if (Climb.isOn)
                options.Add(new CaptureOption("Climb"));
            if (Death.isOn)
                options.Add(new CaptureOption(null, null, "Death"));

            FindFirstObjectByType<SpriteSheetCapture>()
                .Capture(
                    direction,
                    options,
                    int.Parse(FrameSize.text),
                    int.Parse(FrameCount.text),
                    Shadow.isOn
                );
            Close();
        }
