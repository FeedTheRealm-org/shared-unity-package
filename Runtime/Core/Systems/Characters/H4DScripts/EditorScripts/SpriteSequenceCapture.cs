...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class SpriteSequenceCapture : MonoBehaviour
    {
        ...
        {
            UpperBodyClipNames = Directory
                .GetFiles(UpperBodyAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension)
                .ToList();
            LowerBodyClipNames = Directory
                .GetFiles(LowerBodyAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension)
                .ToList();
            ComplexClipNames = Directory
                .GetFiles(ComplexAnimationFolder, "*.anim", SearchOption.AllDirectories)
                .Select(Path.GetFileNameWithoutExtension)
                .ToList();
            ...
            for (var i = 0; i < UpperBodyClipNames.Count; i++)
            {
                if (UpperBodyClipNames[i].EndsWith("U"))
                    UpperBodyClipNames[i] = UpperBodyClipNames[i]
                        .Substring(0, UpperBodyClipNames[i].Length - 1);
            }

            for (var i = 0; i < LowerBodyClipNames.Count; i++)
            {
                if (LowerBodyClipNames[i].EndsWith("L"))
                    LowerBodyClipNames[i] = LowerBodyClipNames[i]
                        .Substring(0, LowerBodyClipNames[i].Length - 1);
            }
        ...
        public void Start()
        {
            ...
            foreach (
                var dropdown in new[]
                {
                    PrimaryAnimationDropdown,
                    SecondaryAnimationDropdown,
                    FrameSizeDropdown,
                    FrameRatioDropdown,
                    ScreenshotIntervalDropdown,
                    ShadowDropdown,
                }
            )
            {
        ...
        public void Load(string path)
        {
            ...
            if (character == null)
                throw new Exception(
                    "Error loading character, please make sure you are loading correct prefab!"
                );
            ...
            Debug.LogWarning(
                "All materials were replaced by [Sprites/Default] to avoid outline artefacts."
            );
        }
        ...
        public void Capture()
        {
            ...
            var upperClips = new List<string>
            {
                UpperBodyClipNames[PrimaryAnimationDropdown.value],
            };
            var lowerClips = new List<string>
            {
                LowerBodyClipNames[SecondaryAnimationDropdown.value],
            };
        ...
        private IEnumerator CaptureFrames(
            List<string> upperClips,
            List<string> lowerClips,
            int frameSize,
            int frameRatio,
            float interval
        )
