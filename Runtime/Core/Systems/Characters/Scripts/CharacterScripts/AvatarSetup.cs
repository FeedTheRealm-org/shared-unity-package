    ...
    public class AvatarSetup : MonoBehaviour
    {
        ...
        public void Initialize(CharacterAppearance appearance, string helmetId)
        {
            if (SpriteCollections.Count == 0)
                throw new Exception("Please set sprite collections for avatar setup.");

            var ears = FindItemSprite(i => i.Ears, appearance.Ears).Sprites[1];

            Head.sprite = FindItemSprite(i => i.Body, appearance.Body)
                .Sprites.Single(i => i.name == "FrontHead");
            ...
            else
            {
                ...
                Hair.color = hair.Tags.Contains("NoPaint")
                    ? (Color32)Color.white
            ...
            Beard.sprite = appearance.Beard.IsEmpty()
                ? null
                : FindItemSprite(i => i.Beard, appearance.Beard).Sprite;
            ...
            else
            {
                ...
                {
                    Hair.sprite = FindItemSprite(i => i.Hair, "Common.Basic.Hair.Default").Sprites[
                        1
                    ];
            ...
            Ears[0].transform.localPosition =
                appearance.Type == 0 ? new Vector3(-1f, 0.5f) : new Vector3(-0.9f, 0.7f);
            Ears[1].transform.localPosition =
                appearance.Type == 0 ? new Vector3(1f, 0.5f) : new Vector3(0.9f, 0.7f);
        }

        private ItemSprite FindItemSprite(
            Func<SpriteCollection, IEnumerable<ItemSprite>> selector,
            string id
        )
        {
