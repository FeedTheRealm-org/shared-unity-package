...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    ...
    public class SpriteSheetCapture : MonoBehaviour
    {
        ...
        )
        {
            ...
            var stateHandler = _character
                .Animator.GetBehaviours<StateHandler>()
            ...
            foreach (var option in options)
            {
                ...
                for (var j = 0; j < frameCount; j++)
                {
                    ...
                    var clip = _character
                        .Animator.GetCurrentAnimatorClipInfo(option.StateU == null ? 2 : 1)[0]
                        .clip;
                    var expressionEvent = clip
                        .events.Where(i =>
                            i.functionName == "SetExpression"
                            && Mathf.Abs(i.time / clip.length - normalizedTime)
                                <= 1f / (frameCount - 1)
                        )
                        .OrderBy(i => Mathf.Abs(i.time / clip.length - normalizedTime))
                        .FirstOrDefault();
            ...
            yield return StandaloneFilePicker.SaveFile(
        ...
        )
        {
            ...
            while (
                _character.Animator.GetCurrentAnimatorClipInfo(stateC == null ? 0 : 2).Length == 0
            )
            {
                yield return null;
            }
